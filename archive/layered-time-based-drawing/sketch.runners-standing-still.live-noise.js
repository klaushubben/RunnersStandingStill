...
function pickFeatures(random) {
    const useSourceTexture = true;
    const sourceTextureMode = previewChoiceParam("sourceTextureMode", "bw-stripes-gradient-slice", [
        "palette-4pt",
        "bw-field",
        "bw-stripes",
    ...
    const decomposeMode = previewChoiceParam(
        ...
        [
            ...
            "byte-mask",
            "byte-mask-dither",
            "byte-mask-dither-levels",
            "ordered-dither",
            "ordered-dither-levels",
        ]
    );
    const ditherPattern = previewChoiceParam("ditherPattern", "bayer-4", [
        "bayer-4",
        "bayer-8",
        "line",
    ]);
    const bwTemperature = previewChoiceParam("bwTemperature", "neutral", ["neutral", "warm", "cool"]);
    const compositeLevels = Math.max(2, Math.min(64, Math.floor(previewNumberParam("compositeLevels", 16))));
    const decomposeBlockChoices = ditherPattern === "line" ? ["8", "16", "32"] : ["8", "16"];
    const decomposeBlockSize = Number(previewChoiceParam("decomposeBlockSize", "16", decomposeBlockChoices));
    const paletteMode = "mono-stripes";
    ...
    const backgroundMode = previewBooleanParam("blackBackground", true) ? "mono-black" : "palette-dark";
    const sourceMode = sourceTextureMode;
    const gradientBandPaletteMode = random.random_choice(["diverging", "split-complement"]);
    ...
    return {
        paletteIndex: 0,
        paletteName: paletteMode === "mono-stripes"
            ? "Mono Stripes"
            : paletteMode === "diverging" ? "Diverging" : "Split Complement",
        ...
        verticalStretch,
        alpha: Math.max(0, Math.min(255, Math.floor(previewNumberParam("alpha", random.random_int(50, 120))))),
        noiseOctaves: 2,
        noiseFalloff: 0.05,
        columnStep: 2,
        sourceMode,
...
globalThis.$features = {
    ...
    "Source Texture": features.sourceTextureMode,
    "Live Mode": "Mono Morph Rows",
    "Draw Mode": features.drawMode,
    "Use Source Texture": "On",
    "Gradient Band Palette": features.gradientBandPaletteMode,
    Decompose: features.decomposeMode,
    "Dither Pattern": features.ditherPattern,
    "B/W Temperature": features.bwTemperature,
    "Composite Levels": features.compositeLevels,
    "Decompose Block Size": features.decomposeBlockSize,
    "Stripe Size": features.stripeSize,
...
sourceTextureMode: ${features.sourceTextureMode}
liveMode: continuous-row-inscriptions
liveEpochSize: ${LIVE_BLOCK_EPOCH_SIZE}
liveRowsPerFrame: ${LIVE_ROWS_PER_FRAME}
liveRowSpan: ${LIVE_ROW_SPAN}
drawMode: ${features.drawMode}
useSourceTexture: true
gradientBandPaletteMode: ${features.gradientBandPaletteMode}
decomposeMode: ${features.decomposeMode}
ditherPattern: ${features.ditherPattern}
bwTemperature: ${features.bwTemperature}
compositeLevels: ${features.compositeLevels}
decomposeBlockSize: ${features.decomposeBlockSize}
stripeSize: ${features.stripeSize.toFixed(2)}
...
function ditherPatternUnit(x, y) {
    ...
    if (features.ditherPattern === "bayer-8") {
...
function decomposedSourceColor(r, g, b, x, y, state) {
    if (features.decomposeMode === "byte-mask" ||
        features.decomposeMode === "byte-mask-dither" ||
        features.decomposeMode === "byte-mask-dither-levels") {
        ...
        if (features.decomposeMode === "byte-mask-dither") {
            return orderedDitherColor(r, g, b, x, y);
        }

        if (features.decomposeMode === "byte-mask-dither-levels") {
    ...
    if (features.decomposeMode === "ordered-dither") {
        return orderedDitherColor(r, g, b, x, y);
    }

    if (features.decomposeMode === "ordered-dither-levels") {
        return orderedDitherLevelsColor(r, g, b, x, y);
    }
...
function sourceUsesGradientSlice() {
    return features.sourceTextureMode === "bw-stripes-gradient-slice";
}

function bwSourceColors(backgroundPalette) {
    if (features.backgroundMode !== "mono-black") {
...
function createTexture() {
    ...
    const palette = features.sourceTextureMode === "palette-4pt"
        ? backgroundPalette
        : generatedPalette();
    const bwColors = bwSourceColors(backgroundPalette);

    if (features.sourceTextureMode === "bw-stripes" || features.sourceTextureMode === "bw-stripes-gradient-slice") {
        if (!monoStripeLayout) {
            let y = 0;
